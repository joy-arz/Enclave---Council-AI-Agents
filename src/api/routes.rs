...
use crate::core::{agent_response, orchestrator};
...
pub async fn test_cli(
    ...
    let provider = factory::create_provider(
        &params.command,
        ws,
        state.config.minimax_api_key.clone(),
        state.config.openai_api_key.clone(),
        state.config.anthropic_api_key.clone(),
        state.config.openrouter_api_key.clone(),
        Some(state.config.minimax_model.clone()),
        Some(state.config.minimax_base_url.clone()),
        Some(state.config.openrouter_model.clone()),
        Some(state.config.openrouter_base_url.clone()),
    ...
    match provider
        .call_model(
            "test",
            "ping",
            Some("respond with 'pong' if you are working"),
            0.7,
            10,
            None,
        )
...
pub async fn handle_enclave(
    ...
    let autonomous = params.autonomous.unwrap_or(state.config.autonomous_mode);
    let ws = params
        .workspace_dir
        .map(std::path::PathBuf::from)
        .unwrap_or_else(|| state.config.workspace_dir.clone());
    ...
    let ws_for_providers = if worktree.is_some() {
        execution_ws
    } else {
        ws.clone()
    };
    ...
    let prev_history = if has_session {
        state.session_store.get_history(&session_id).await
    } else {
        vec![]
    };
    ...
    let s_bin = params
        .strategist_binary
        .unwrap_or_else(|| state.config.strategist_binary.clone());
    let c_bin = params
        .critic_binary
        .unwrap_or_else(|| state.config.critic_binary.clone());
    let o_bin = params
        .optimizer_binary
        .unwrap_or_else(|| state.config.optimizer_binary.clone());
    let ct_bin = params
        .maintainer_binary
        .unwrap_or_else(|| state.config.contrarian_binary.clone());
    let j_bin = params
        .judge_binary
        .unwrap_or_else(|| state.config.judge_binary.clone());
    ...
    let mut agents = vec![
        roles::strategist(
            ...
            state.config.max_tokens_per_agent,
        ),
        roles::critic(
            critic_provider,
            "cli",
    ...
    let mut judge = judge_agent::new(judge_provider, "cli", state.config.default_temperature, 1000);
    ...
    let mut orchestrator_inst = orchestrator::new(
        agents,
        judge,
        params.rounds.unwrap_or(state.config.max_rounds),
        auto_rounds,
        20,
        ws_for_providers,
    );
    ...
    tokio::spawn(async move {
        ...
        let _ = tx
            .send(
                Event::default()
                    .event("session_info")
                    .data(serde_json::json!({"session_id": sid_clone}).to_string()),
            )
            .await;

        // restore session history if available
        if !prev_history.is_empty() {
            let history_preview: Vec<String> = prev_history
                .iter()
                .map(|m| {
        ...
        store_clone
            .add_message(&session_id, user_query_response)
            .await;

        let _ = orchestrator_inst
            .run_council(&query, |resp| {
            ...
            .inspect_err(|e| {
                ...
                tokio::spawn(async move {
                    let _ = logger_err
                        .log(&format!("fatal orchestrator error: {}", err_msg))
                        .await;
                });
            });
    ...
    let stream = stream::unfold(rx, |mut rx| async move {
        rx.recv()
            .await
            .map(|event| (Result::<Event, Infallible>::Ok(event), rx))
    });
...
pub async fn apply_change(
    ...
    let ws = &state.config.workspace_dir;
    ...
    if target_path.is_absolute() || target_path.components().any(|c| c.as_os_str() == "..") {
        return Json(
            serde_json::json!({"status": "error", "message": "invalid path: path traversal detected"}),
        );
    }
            ...
            match ws.canonicalize() {
                Ok(ws_resolved) => {
                    if !resolved.starts_with(&ws_resolved) {
                        return Json(
                            serde_json::json!({"status": "error", "message": "invalid path: path escapes workspace"}),
                        );
                    }
                }
                Err(e) => {
                    return Json(
                        serde_json::json!({"status": "error", "message": format!("failed to resolve workspace: {}", e)}),
                    );
                }
            }
        }
                    ...
                    match ws.canonicalize() {
                        Ok(ws_resolved) => {
                            if !parent_canonical.starts_with(&ws_resolved) {
                                return Json(
                                    serde_json::json!({"status": "error", "message": "invalid path: parent directory escapes workspace"}),
                                );
                            }
                        }
                        Err(_) => {
    ...
    if let Some(parent) = full_path.parent() {
        if let Err(e) = tokio::fs::create_dir_all(parent).await {
            return Json(
                serde_json::json!({"status": "error", "message": format!("failed to create directories: {}", e)}),
            );
        }
    }
...
pub async fn get_session_history(
    ...
    Json(state.session_store.get_history(&session_id).await)
}

pub async fn list_sessions(
    State(state): State<AppState>,
) -> Json<Vec<crate::api::sessions_mod::SessionSummary>> {
    Json(state.session_store.list_sessions().await)
}
...
pub async fn delete_session(
    ...
    Json(
        serde_json::json!({"status": if deleted { "success" } else { "error" }, "deleted": deleted}),
    )
}
