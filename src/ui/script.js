...
// Load API key status and update dropdown indicators
async function loadApiKeyStatus() {
  try {
    ...
    api_key_status = {
      minimax: config.has_minimax_key,
      openai: config.has_openai_key,
      anthropic: config.has_anthropic_key,
      openrouter: config.has_openrouter_key,
    };

    // Update all dropdowns with API key availability indicators
    Object.keys(bin_inputs).forEach((role) => {
      const select = bin_inputs[role];
      Array.from(select.options).forEach((opt) => {
        const provider = opt.value;
        if (provider !== "gemini" && provider !== "qwen") {
          const hasKey = api_key_status[provider];
          if (!hasKey) {
            // Append "No API key" indicator if not already present
            if (!opt.text.includes("(No API key)")) {
              opt.text = opt.text + " (No API key)";
              opt.style.color = "#71717a";
            }
          } else {
            // Remove indicator if key is available
            opt.text = opt.text.replace(" (No API key)", "");
            opt.style.color = "";
          }
        }
      });
...
document
  ...
  .addEventListener("click", async () => {
    ...
    showToast(
      "Session cleared. You can continue by asking another question.",
      "success",
    );
  });
...
document
  .getElementById("saved-sessions-btn")
  .addEventListener("click", async () => {
    openSessionsModal();
  });
...
async function loadSessionsList() {
  ...
  sessionsList.innerHTML =
    '<div class="empty-state"><div class="empty-sessions-icon">⏳</div><div>Loading sessions...</div></div>';
...
function check_silence() {
  ...
  if (elapsed > 60000) {
    ...
    if (minutes >= 60) {
      update_status(
        `Silent for ${Math.floor(minutes / 60)}h ${minutes % 60}m...`,
        true,
      );
    } else {
  ...
  if (elapsed > 300000) {
    ...
    showToast(
      "Enclave seems stuck. May have failed silently.",
      "warning",
      8000,
    );
  }
...
function start_debate(
  ...
  event_source.onerror = (e) => {
    ...
    if (lead_engineer_received) {
      ...
      done_div.style =
        "text-align: center; color: var(--accent); font-size: 0.75rem; font-weight: 600; margin: 24px auto; padding: 12px 24px; background: var(--accent-muted); border: 1px solid var(--accent); border-radius: var(--radius); max-width: 300px; text-transform: uppercase; letter-spacing: 0.08em;";
...
function append_message(
  ...
  try {
    if (agent_lower === "lead.engineer") {
      ...
      const status_color =
        json.final_decision === "FINISHED"
          ? "var(--accent)"
          : json.final_decision === "CONTINUE"
            ? "var(--primary)"
            : "var(--warning)";
      body.innerHTML = `
      ...
      body.innerHTML =
        tool_indicators + escapeHtml(content).replace(/\n/g, "<br>");
    } else {
      ...
      if (proposals.length > 0) {
        ...
        body.innerHTML =
          tool_indicators +
          escapeHtml(clean_content.trim()).replace(/\n/g, "<br>");
        ...
        proposals.forEach((p) => {
          ...
          apply_btn.onclick = () =>
            apply_proposed_change(p.path, p.content, apply_btn);
  ...
  if (terminal_output) {
    ...
    term_details.style =
      "margin-top: 16px; font-size: 0.75rem; border-top: 1px solid var(--border); padding-top: 12px;";
    const term_summary = document.createElement("summary");
    term_summary.textContent = "Terminal Logs & Thoughts";
    term_summary.style =
      "cursor: pointer; color: var(--text-muted); font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;";

    const term_pre = document.createElement("pre");
    term_pre.style =
      "margin-top: 8px; padding: 12px; background: var(--bg); border-radius: var(--radius-sm); max-height: 400px; overflow-y: auto; overflow-x: hidden; color: #94a3b8; font-family: 'Fira Code', monospace; white-space: pre-wrap; overflow-wrap: anywhere;";
